...
plotConseq <- function(dets.ordered){
  ...
  plot(dets[dets$ind==ind1,]$fullTimeFromClipStart, 1:nrow(dets[dets$ind==ind1,]), bty="l", pch=21, las=1, type="b", lwd=1.75, xlab="Elapsed time", ylab="Cumulative # chest beats", main=paste("consecutive chest beats on", start.date), xlim=c(min(dets$fullTimeFromClipStart)-1, max(dets$fullTimeFromClipStart)+1), ylim=c(0,max(table(dets$ind))))
